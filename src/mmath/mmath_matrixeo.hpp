...
namespace mmath {
    ...
    namespace meotypes {
        ...
        class matrix_axis_switch {
          ...
          public:
            matrix_axis_switch(meotypes::matrix_axis<T> axis1, matrix_axis<T> axis2)
                : op_axis1(axis1.get_axis())
                , op_axis2(0)
                , axis_swap(axis2.get_axis())
                , axis_result(0) {}

            matrix_axis_switch(meotypes::matrix_axis_added<T> op1, matrix_axis<T> op2)
            ...
            matrix_axis_switch(meotypes::matrix_axis_switch<T> op1, matrix_axis<T> op2)
...
#pragma region "| Operator"
    ...
    matrix_base<T>& operator|(matrix_base<T>& m, meotypes::matrix_axis_switch<W> axis) {
        ...
        impl::axis_operation<
            int>(m, axis.get_op_axis1(), axis.get_op_axis2(), axis.get_axis_result(), axis.get_axis_swap(),
    ...
    matrix_base<T>& operator|(matrix_base<T>& m, meotypes::matrix_axis_switch_generic<W, S> axis) {
        ...
        impl::axis_operation<
            S>(m, axis.get_op_axis1(), axis.get_op_axis2(), axis.get_axis_result(), axis.get_axis_swap(), axis.is_sub(),
               axis.is_mul_axis1(), std::move(axis.get_multiplicand()), axis.get_type());
        return m;
    }
...
#pragma region "<=> Operator"
    ...
    meotypes::matrix_axis_switch<T> operator<=>(meotypes::matrix_axis<T> a, meotypes::matrix_axis<T> b) {
        return meotypes::matrix_axis_switch<T>{a, b};
    }
...
#pragma region ">> Operator"
    ...
    meotypes::matrix_axis_switch_generic<T, S> operator>>(meotypes::matrix_axis_multiplied<T, S> a,
        ...
        return meotypes::matrix_axis_switch_generic<T, S>{a, b};
    }
...
#pragma region "+ and - Operators"
    ...
    meotypes::matrix_axis_added<T> operator+(meotypes::matrix_axis<T> a, meotypes::matrix_axis<T> b) {
        return meotypes::matrix_axis_added<T>{a, b};
    }

    template <meotypes::matrix_axis_type T, typename S>
    meotypes::matrix_axis_added_generic<T, S> operator+(meotypes::matrix_axis_multiplied<T, S> a,
                                                        meotypes::matrix_axis<T> b) {
        return meotypes::matrix_axis_added_generic<T, S>{a, b};
    }

    template <meotypes::matrix_axis_type T, typename S>
    meotypes::matrix_axis_added_generic<T, S> operator+(meotypes::matrix_axis<T> a,
                                                        meotypes::matrix_axis_multiplied<T, S> b) {
        return meotypes::matrix_axis_added_generic<T, S>{b, a};
    }

    template <meotypes::matrix_axis_type T>
    meotypes::matrix_axis_added<T> operator-(meotypes::matrix_axis<T> a, meotypes::matrix_axis<T> b) {
        return meotypes::matrix_axis_added<T>{a, b, true};
    }

    template <meotypes::matrix_axis_type T, typename S>
    meotypes::matrix_axis_added_generic<T, S> operator-(meotypes::matrix_axis_multiplied<T, S> a,
                                                        meotypes::matrix_axis<T> b) {
        return meotypes::matrix_axis_added_generic<T, S>{a, b, true};
    }
...
#pragma region "* Operator"
    ...
    meotypes::matrix_axis_multiplied<T, S> operator*(S a, meotypes::matrix_axis<T> b) {
        return meotypes::matrix_axis_multiplied<T, S>{std::move(a), b};
    }

    template <meotypes::matrix_axis_type T, typename S>
    meotypes::matrix_axis_multiplied<T, S> operator*(meotypes::matrix_axis<T> a, S b) {
        return meotypes::matrix_axis_multiplied<T, S>{std::move(b), a};
    }
...
#pragma region "/ Operator"
    template <meotypes::matrix_axis_type T, typename S>
    meotypes::matrix_axis_multiplied<T, S> operator/(meotypes::matrix_axis<T> a, S b) {
        return meotypes::matrix_axis_multiplied<T, S>{1 / std::move(b), a};
    }
#pragma endregion "/ Operator"
